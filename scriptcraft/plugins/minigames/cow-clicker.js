...
events.playerInteractEntity( function( event ) {
    ...
    if (event.getHand() == org.bukkit.inventory.EquipmentSlot.OFF_HAND) {
        echo( event.player, 'off hand event, ignore');
        return;
    }

    echo( event.player, 'into event handler');
    ...
    if (clickedEntity instanceof org.bukkit.entity.Cow)
    {
        ...
        Bukkit.dispatchCommand(player, 'me again clicked a cow!');
        sound(org.bukkit.Sound.UI_BUTTON_CLICK,1,1);
        setTimeout(function(){
            sound(org.bukkit.Sound.ENTITY_COW_HURT,10,0.85) 
        },200);
    };
});
